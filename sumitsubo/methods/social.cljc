(ns sumitsubo.methods.social
  "social.cljc — sumitsubo DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from sumitsubo's own
...
(defn draft-observation-post
  "A dry-run observation post over sumitsubo's own domain (Cleanroom CAD interop for kami-engine-sdk — a geometry kernel + open-format export/import (DXF/SVG/OBJ/glTF/IFC-subset/STEP-subset;…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "sumitsubo R0: live social posting is Council Lv6+ + operator + "
